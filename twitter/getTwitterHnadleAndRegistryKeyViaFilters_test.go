...
func TestGetTwitterHnadleAndRegistryKeyViaFilters(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("GetTwitterHandleAndRegistryKeyViaFilters:%s", tt.name), func(t *testing.T) {
			var (
				w   want
				err error
			)
			if w.registry, w.handle, err = twitter.GetTwitterHandleAndRegistryKeyViaFilters(
				conn,
				tt.verifiedPubKey,
			); err != nil {
				t.Fatalf("%s: error: %s\n", tt.name, err.Error())
